...
1. **Frontend Desktop (`flutter_app/`)**:
   ...
   - **Atualizador (`update_service.dart` + aba "Atualizações") — invariante desde a v1.1.0**:
     ...
     - **A entrega do instalador passa por `cmd.exe /c start "" /min powershell …`** (`comandoDoHandoff`). Duas armadilhas, medidas nas condições reais do app: (1) entregar o PowerShell direto em `ProcessStartMode.detached` não roda nada — o modo destacado tira o console do filho e o PowerShell 5.1 é executável de console; (2) entregar com `start /b` funciona num terminal, mas **não** no aplicativo: `/b` herda o console do pai, um app GUI não tem console, e o processo morre junto com o `exit(0)`. O `/min` dá console próprio ao filho — é o que permite ao instalador trabalhar depois de o app fechar. O teste "o script entregue roda de verdade" em `test/update_service_test.dart` cobre a armadilha (1); a (2) só apareceu na verificação ao vivo.
